...
%% 计算实际轨迹和目标轨迹之间的RMSE，注意这里是累积的RMSE，平均值将会在test_trajectory.m中计算
global rmse_p
global rmse_v
global rmse_yaw
% initialize rmse
...
rmse_p = rmse_p + sqrt(sum((s(1:3)-s_des(1:3)).^2));
rmse_v = rmse_v + sqrt(sum((s(4:6)-s_des(4:6)).^2));
rmse_yaw = rmse_yaw + sqrt((s(10)-s_des(10))^2);
end
